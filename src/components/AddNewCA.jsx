...
export const AddNewCA = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("/api/user/addCa", {
        montant,
        dateDuJour,
      });
  ...
  return (
    <div className="flex flex-col items-center m-5">
      ...
      <h3>
        Enregistrer mon chiffre d'affaire de {actualMonth} {year}
      </h3>
      <div className="flex justify-center flex-col w-[100%] items-center">
        <form
          onSubmit={handleSubmit}
          className="bg-[#F99884] flex flex-col w-[75%] items-center gap-3"
        >
          ...
          <input
            ...
            className="w-[50%]"
          />
          <button type="submit" className="w-[10em] border mb-3">
            Enregistrer
          </button>
        </form>
        {message && (
          <p
            className={`flex justify-center p-3 text-white w-[75%] ${
              valide ? "bg-green-500" : "bg-red-600"
            }`}
          >
            {message}
          </p>
